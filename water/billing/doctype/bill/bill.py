...
class Bill(Document):
	...
	def send_bill_notification(self):
		...
		billing_period_doc = frappe.get_list("Billing Period",fields = ['name_of_billing_period'])
		billing_period = billing_period_doc[0]['name_of_billing_period']
  
		#generate message
		message = "Dear customer here is your billing details for {}:Prev reading={} units, current reading={} units, consumption={} units, new bill=Ksh.{}. Your outstanding balance \
			is {} and your account balance is {}.".format(billing_period,self.last_meter_reading,self.current_meter_reading,self.consumption,self.total_bill,outstading_balance,\
			account_balance)
